...
r = f < g
plot(r, -10, 10, -10, 10, W=2^9, H=2^9, show_red=true)  # xmin, xmax, ymin, ymax, pixels_wide, pixels_height
gcf()
```
...
produced by `plot(r::Predicate)`. Adding layers depends on the
underlying plotting package. Rather than do that, we can plot multiple graphs using "or:"
